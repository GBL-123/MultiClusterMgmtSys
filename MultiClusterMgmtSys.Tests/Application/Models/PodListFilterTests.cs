...
public class PodListFilterTests
{
    ...
    private static readonly List<PodListViewModel> _Pods =
    [
    ...
    {
        var result = PodListFilter.Apply(_Pods, "APP", null, null).ToList();
    ...
    {
        var result = PodListFilter.Apply(_Pods, null, "异常", null).ToList();
    ...
    {
        var byName = PodListFilter.Apply(_Pods, null, null, "web-").ToList();
        Assert.Equal(2, byName.Count);

        var byNode = PodListFilter.Apply(_Pods, null, null, "node-c").ToList();
        Assert.Single(byNode, p => p.Name == "db-0");

        var byIp = PodListFilter.Apply(_Pods, null, null, "10.1.1.1").ToList();
    ...
    {
        var result = PodListFilter.Apply(_Pods, "batch", "失败", null).ToList();
    ...
    {
        var options = PodListFilter.StatusOptions(_Pods, null, null);
    ...
    {
        var options = PodListFilter.StatusOptions(_Pods, "app", null);
