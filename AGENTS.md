...
- 看板页(`/dashboard`)是聚合快照而非表格页:样式在 `app.css` **第 15 节**,**不**登记进上面的表格 flex-fill 规则组。顶部是**看板板面卡**(`.dashboard-board`,默认 elevation 的 `MudPaper`):报头区 `.dashboard-masthead`(标题 28–32px + 新鲜度 + 刷新入口)与 KPI 统计带之间以 **2px 墨线**分区(该线是 `.dashboard-stat-bar` 的 `border-top`);KPI 带 `auto-fit minmax(200px,1fr)` 窄屏自动折行、1px 发丝线槽、每格 `01`–`05` 编号 + 11px 标签 + **56px** 大数字 + 注释,**不要**给它 `overflow: hidden`。主体两行:`分组健康`/`版本分布`/`节点就绪` 三张指标卡一行(`.dashboard-metrics`,三列),`需要关注`/`最近操作` 两张清单卡一行(`.dashboard-lists`,两列),同行卡片由 grid 拉伸为同高;≤1280px 单列。卡壳 `.dashboard-card` + 纸色卡头 `.dashboard-card-head`(13px 墨色等宽标题 + 1px 墨色下沿线),行类 `.dashboard-attention-row`/`.dashboard-breakdown-row`/`.dashboard-activity-row`(发丝线分隔 + hover 暖灰),计量条 `.dashboard-meter`(6px;`.is-online` 绿 / `.is-offline` 红)。`节点就绪` 卡(`DashboardNodeReadinessCard`)取各集群最近快照的 `ReadyNodes`/`NotReadyNodes` 之和(**零 K8s 调用**),无快照时显示紧凑空态。大号数字 `.dashboard-stat-value` + `.font-grotesk`(56px、-0.04em;`html ` 前缀压过 Mud 的 body1 字号;不要用 `vw` 缩放),数据行 `.font-mono`,状态计数仅在非零时着色(在线绿/离线红/未知琥珀)。**注意**:组件属性(如 `MudText` 的 `Class`)不接受字面量与 C# 表达式混排(编译错误 RZ9986),条件类名必须整体在 `@code` 中算好。
