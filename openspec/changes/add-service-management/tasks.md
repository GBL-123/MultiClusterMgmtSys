...
- [x] 2.2 GetNamespacesAsync / ListSvcsAsync(SvcQueryRequest,命名空间过滤)
- [x] 2.3 GetSvcAsync(SvcKeyRequest)→ SvcDetailViewModel
- [x] 2.4 GetSvcEndpointsAsync:EndpointSlice 优先(label `kubernetes.io/service-name`),仅 404 降级 CoreV1 Endpoints,统一折叠为 SvcEndpointViewModel
- [x] 2.5 CreateSvcFromYamlAsync:反序列化(失败抛中文 ValidationException)+ metadata.namespace 必填 + 审计(创建)
- [x] 2.6 UpdateSvcFromYamlAsync:读回 existing → clusterIP/clusterIPs/ipFamilies 守卫(不同抛中文校验异常,省略补齐)→ 补 resourceVersion/uid、保留 status → Replace + 审计(修改)
- [x] 2.7 DeleteSvcAsync:删除 + 审计(删除,目标含 ns/name/集群名)
...
- [x] 3.2 `Components/Svcs/Pages/Services.razor`(路由 /services、/services/{ClusterId:int}):集群侧栏 + 状态徽章 + 命名空间过滤 + 名称搜索 + 列表表格 + 新建对话框入口(Admin)
- [x] 3.3 `Components/Svcs/Shared/SvcListTable.razor`:名称链接/命名空间 chip/类型(mono)/端口列(每端口一行,kubectl 语法 + targetPort,>3 截断 +N)/对外入口列/ClusterIP(None 形态)/创建时间/操作列;ExternalName 端口格 `[ 暂无 ]`
- [x] 3.4 `Components/Svcs/Shared/SvcListFilterBar.razor`(命名空间下拉 + 名称搜索,含端口搜索文本框,若实现)
- [x] 3.5 `Components/Svcs/Shared/`:SvcYamlViewCard / SvcYamlEditCard(yaml-textarea 模式)/ CreateSvcDialog(YAML 模板)
- [x] 3.6 `Components/Svcs/Pages/SvcDetail.razor`(路由 /services/{ClusterId:int}/{Namespace}/{Name}):YAML 视图 + 端口表 + Endpoints 卡(.status-badge Ready 徽章)+ 详情工具栏(刷新/编辑/删除,Admin 门控);不存在空态、不可达降级
- [x] 3.7 `Components/Svcs/Pages/EditSvcYaml.razor`:编辑页复用 SvcYamlEditCard,异常经 ExHandler.HandleAsync 呈现
...
- [x] 3.11 CreateSvcDialog 增加 Service 类型选择与按类型 YAML 模板(实施期反馈)
...
- [x] 4.3 UpdateSvcFromYamlAsync 守卫测试:修改 clusterIP 抛中文 ValidationException 且不触 K8s、省略补齐成功、可变字段替换成功
