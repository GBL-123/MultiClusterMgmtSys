...
- **List page visual alignment** — the node list right pane mirrors `Clusters.razor`: title row (h5 "节点管理" + cluster context chips) and filter bar share one `MudPaper`; the table gains sortable headers, `FixedHeader`, and a loading content (client-side paging retained).
