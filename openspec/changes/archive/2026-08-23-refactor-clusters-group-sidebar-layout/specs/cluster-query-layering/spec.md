# cluster-query-layering (delta: refactor-clusters-group-sidebar-layout)

## Purpose

Existing capability unchanged except for one sentential expansion: `ClusterPageQuery.GroupId` gains a "ungrouped" sentinel (`0`) that the repository MUST translate to a `GroupId IS NULL` predicate. The rest of the cluster-query-layering contract is preserved verbatim. This delta spec captures only the changed requirement.
