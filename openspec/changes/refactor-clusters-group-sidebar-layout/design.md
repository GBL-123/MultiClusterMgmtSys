...
- **Sidebar count sync after batch move:** after a successful `MoveClustersToGroupAsync`, the sidebar's `ClusterCount` values are stale. Parent re-fetches `groups = await GroupService.GetGroupsAsync()` to update counts.
