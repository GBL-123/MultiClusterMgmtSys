...
为 apps/v1 四类工作负载（Deployment / StatefulSet / DaemonSet / ReplicaSet）提供集群内管理闭环：侧边导航「工作负载管理」组下每类一个列表页（集群侧栏 + 命名空间/名称过滤 + 就绪度与滚动状态列）与一个详情页（YAML 视图/编辑、扩缩容、重启、删除）。写操作按类型可用性矩阵约束（扩缩容不含 DaemonSet，重启不含 ReplicaSet）并以 Admin 角色门控，查看对所有登录用户开放；YAML 编辑采用只覆盖 spec 的乐观并发策略，全部写操作成功后写入审计。
