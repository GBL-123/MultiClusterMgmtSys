...
为 Kubernetes Service 提供跨集群管理能力:导航入口(网络管理分组)、以端口为一等公民的服务列表(类型筛选、名称与端口搜索、对外入口列、特殊服务形态)、详情页(YAML 视图、端口表、经 EndpointSlice 优先/旧 Endpoints 降级读取的后端列表),以及 YAML 创建(按类型外置模板)、YAML 编辑(不可变字段守卫)与删除;写操作仅 Admin 可见,创建/修改/删除成功后写入审计。
