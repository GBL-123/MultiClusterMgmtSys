...
基于 ASP.NET Core Identity（`AddIdentityCore<ApplicationUser>` + `AddEntityFrameworkStores`）的账号生命周期：Member 自助注册、Admin 后台账号 CRUD、自助资料与密码修改、Identity 表与密码强度策略。所有账号操作（注册 / 新建 / 改密 / 重置）由 `UserManager` / `SignInManager` / `RoleManager` 统一管理，密码通过自定义 `IPasswordValidator<ApplicationUser>` (`AlphanumericPasswordValidator`) 强制强度。
