...
Define the redesigned Kubernetes node management UI: a two-column node list page (`/nodes` and `/nodes/{ClusterId}`) with a collapsible cluster-group sidebar, client-side filtering (name / role / status / schedulability), a six-column node table, session-scoped cluster-selection memory via `ClusterSelectionState`, and a card-composed node detail page at `/nodes/{ClusterId}/{NodeName}` — all built from shared components under `Components/Nodes/Shared/` using the project's path-matched namespaces.
