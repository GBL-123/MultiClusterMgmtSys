...
Define the contract for service-layer input/output types in this repository: public service methods accept frontend inputs as `Application/Requests` Request objects (single primitive and framework-type returns exempted), return display data as `Application/ViewModels` ViewModels (never `Domain/Entities` entities), resolve operator identity from `HttpContext` via `IHttpContextAccessor` instead of frontend-passed parameters, never expose MudBlazor framework types in service/repository signatures, and place each class in its owning project folder (`Application/Requests` inputs, `Application/ViewModels` outputs, `Application/Models` frontend-only state).
